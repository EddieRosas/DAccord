...
class ChannelList extends React.Component {
    ...
    render() {
        if (Object.values(this.props.servers).length === 0)
            return null
        const isOwner = this.props.servers[this.props.match.params.serverId].owner_id === this.props.currentUserId;
        const channelItems =
            this.props.channels.length > 0 ?
                this.props.channels.map(channel => <ChannelListItemContainer key={channel.id} channel={channel} isOwner={isOwner} />)
                : null;
